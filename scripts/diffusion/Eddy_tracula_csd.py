...
work_dir = os.path.abspath('/om/scratch/Fri/ksitek/Eddy_tracula_csd')

"""
brun = 'dwi_1k'

data_dir = '/mindhive/xnat/data/isn'
tracula_dir = '/om/user/ksitek/isn/tracula'+brun
subject_list = ['GAB_ISN_012','GAB_ISN_014']#,'GAB_ISN_017'

infosource = Node(interface=IdentityInterface(fields=['subject_id']),
                     name="infosource")
infosource.iterables = ('subject_id',subject_list)

templates = {'dwi':   '/mindhive/xnat/data/isn/{subject_id}/diffusion/{PE}_001.nii.gz',
             'bvals': '/mindhive/xnat/data/isn/{subject_id}/diffusion/{PE}_001.bvals',
             'bvecs': '/mindhive/xnat/data/isn/{subject_id}/diffusion/{PE}_001.bvecs'}

"""
tracula_dir = '/om/project/voice/processedData/tracula/'
subject_list = sorted([os.path.basename(x) for x in glob('/om/project/voice/processedData/openfmri/voice85*')])
infosource = Node(interface=IdentityInterface(fields=['subject_id']),
                     name="infosource")
infosource.iterables = ('subject_id',subject_list)
...
node2.plugin_args = {'sbatch_args': '--qos=gablab --gres=gpu:1',
                      'overwrite': True}
...
tracker.plugin_args = {'sbatch_args': '--qos=gablab --mem=%dG -N 1 -c %d' % (10 * num_threads, #3 * num_th
                                                                                    num_threads),
                       'overwrite': True}
